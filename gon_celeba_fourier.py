...
if __name__ == '__main__':
    ...
    gon_shape = [mapping_size + num_latent] + [hidden_features] * num_layers + [n_channels]
    ...
    np.random.seed(47)
    B_gauss = np.random.randn((mapping_size // 2, 2))
    np.random.seed(None)

    B_gauss = torch.tensor(B_gauss, device=device) * 5

    # (B, H x W, 2)
    c = torch.stack([get_mgrid(img_size, 2) for _ in range(batch_size)]).to(device)  # coordinates

    # (B, H x W, hidden_features)
    c = coor_mapping(c, B_gauss)
    ...
    for step in range(opt.steps):
        ...
        z = torch.randn(batch_size, 1, num_latent).to(device).requires_grad_()
